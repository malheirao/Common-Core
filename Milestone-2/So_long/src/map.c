...
/*   Updated: 2025/11/05 19:03:42 by lmanzani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_line(char ***map, char *line, int *line_count, int *lines_allocated, int fd)
{
	int	len;
...
char	**load_map(const char *filename)
{
	...
	while (line)
	{
		if (!process_line(&map, line, &line_count, &lines_allocated, fd))
			return (NULL);
		line = get_next_line(fd);
	}
	map[line_count] = NULL;
